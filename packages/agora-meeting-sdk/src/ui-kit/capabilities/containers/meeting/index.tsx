...
import { MettingGoods } from '../goods';
...
export const MeetingContainer = observer(() => {
  const { renderLayout, setLayout, memberVisible,goodsVisible } = useUIStore();
  ...
  const drawerClsLeft = classnames('drawer-left', {
    'drawer-left-show': goodsVisible,
  });
  
  const [height, setHeight, leftHeight] = useState('100%');
  ...
  return (
    <div className="metting">
	  <section className={drawerClsLeft}>
	    {goodsVisible ? (
	      <div className="drawer-item" style={{ height: leftHeight }}>
	        <MettingGoods></MettingGoods>
	      </div>
